var V=Object.defineProperty;var O=(f,t,e)=>t in f?V(f,t,{enumerable:!0,configurable:!0,writable:!0,value:e}):f[t]=e;var o=(f,t,e)=>O(f,typeof t!="symbol"?t+"":t,e);import{M as q,U as z,S as H,O as W}from"./orbit_panel-Bzh63E9W.js";const Y=`// WGSL translation of the Black Hole Ray Tracing shader
//
// =====================================================================================
...
`,te=function(f,t){const e=new XMLHttpRequest;e.open("GET","/black-hole/"+f),e.responseType="arraybuffer",e.onload=()=>{if(e.status!==200){console.error("Failed to load rocket mesh binary:",f,"status:",e.status);return}const r=new DataView(e.response),n=r.getUint32(0,!0),a=r.getUint32(Uint32Array.BYTES_PER_ELEMENT,!0);let i=2*Uint32Array.BYTES_PER_ELEMENT;const p=new Float32Array(n);for(let s=0;s<n;++s)p[s]=r.getFloat32(s*Float32Array.BYTES_PER_ELEMENT+i,!0);i+=n*Float32Array.BYTES_PER_ELEMENT;const c=new Uint32Array(a);for(let s=0;s<a;++s)c[s]=r.getUint32(s*Uint32Array.BYTES_PER_ELEMENT+i,!0);t(p,c)},e.send()},k=function(f,t){const e=f.createTexture({size:[1,1,1],format:"rgba8unorm",usage:GPUTextureUsage.TEXTURE_BINDING|GPUTextureUsage.COPY_DST}),r=new Uint8Array([255,255,255,255]);f.queue.writeTexture({texture:e},r,{bytesPerRow:4},[1,1,1]);const n={texture:e},a=new Image;return a.addEventListener("load",async()=>{try{const i=await createImageBitmap(a),p=Math.floor(Math.log2(Math.max(i.width,i.height)))+1,c=f.createTexture({label:t,size:[i.width,i.height,1],mipLevelCount:p,format:"rgba8unorm",usage:GPUTextureUsage.TEXTURE_BINDING|GPUTextureUsage.COPY_DST|GPUTextureUsage.RENDER_ATTACHMENT}),s=document.createElement("canvas"),d=s.getContext("2d");if(!d)throw new Error("Could not get 2D context");let l=i.width,v=i.height,_=0;for(;l>=1&&v>=1;){s.width=l,s.height=v,d.drawImage(i,0,0,l,v);const x=await createImageBitmap(s);if(f.queue.copyExternalImageToTexture({source:x},{texture:c,mipLevel:_},[l,v]),l===1&&v===1)break;l=Math.max(1,Math.floor(l/2)),v=Math.max(1,Math.floor(v/2)),_++}n.texture=c}catch(i){console.error("Failed to load/generate mipmaps for rocket texture:",t,i)}}),a.src="/black-hole/"+t,n};class re{constructor(t,e){o(this,"model");o(this,"cameraView");o(this,"device");o(this,"rocketUniformBuffer");o(this,"exhaustUniformBuffer");o(this,"envMapUniformBuffers",[]);o(this,"baseColorTextureWrapper");o(this,"occlusionRoughnessMetallicTextureWrapper");o(this,"normalMapTextureWrapper");o(this,"envMapBindGroups",null);o(this,"cachedEnvMapSkyTexture",null);o(this,"rocketVertexBuffer",null);o(this,"rocketIndexBuffer",null);o(this,"rocketIndexCount",0);o(this,"exhaustVertexBuffer",null);o(this,"exhaustIndexBuffer",null);o(this,"exhaustIndexCount",0);o(this,"envMapTexture");o(this,"rocketBindGroupLayout");o(this,"rocketPipeline");o(this,"exhaustBindGroupLayout");o(this,"exhaustPipelineFront");o(this,"exhaustPipelineBack");o(this,"mipmapBindGroupLayout");o(this,"mipmapPipeline");o(this,"exhaustBindGroup");o(this,"rocketBindGroup",null);o(this,"cachedBaseColor",null);o(this,"cachedMetallic",null);o(this,"cachedNormal",null);this.model=t,this.cameraView=e,this.device=e.device,this.rocketUniformBuffer=this.device.createBuffer({size:80,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST}),this.exhaustUniformBuffer=this.device.createBuffer({size:128,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST}),this.envMapUniformBuffers=[];for(let r=0;r<6;++r)this.envMapUniformBuffers.push(this.device.createBuffer({size:432,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST}));this.baseColorTextureWrapper=k(this.device,"rocket_base_color.png"),this.occlusionRoughnessMetallicTextureWrapper=k(this.device,"rocket_occlusion_roughness_metallic.png"),this.normalMapTextureWrapper=k(this.device,"rocket_normal.png"),this.envMapBindGroups=null,this.cachedEnvMapSkyTexture=null,this.rocketVertexBuffer=null,this.rocketIndexBuffer=null,this.rocketIndexCount=0,this.exhaustVertexBuffer=null,this.exhaustIndexBuffer=null,this.exhaustIndexCount=0,this.createEnvMapTexture(),this.createRocketPipeline(),this.createExhaustPipeline(),this.createMipmapPipeline(),te("rocket.dat",(r,n)=>this.createRocketBuffers(r,n)),this.createExhaustBuffers()}createEnvMapTexture(){this.envMapTexture=this.device.createTexture({label:"RocketEnvMapCubeTexture",size:[E,E,6],mipLevelCount:L,format:"rgba16float",usage:GPUTextureUsage.TEXTURE_BINDING|GPUTextureUsage.RENDER_ATTACHMENT})}createRocketPipeline(){this.rocketBindGroupLayout=this.device.createBindGroupLayout({label:"RocketBindGroupLayout",entries:[{binding:0,visibility:GPUShaderStage.VERTEX|GPUShaderStage.FRAGMENT,buffer:{type:"uniform"}},{binding:1,visibility:GPUShaderStage.FRAGMENT,sampler:{type:"filtering"}},{binding:2,visibility:GPUShaderStage.FRAGMENT,texture:{sampleType:"float",viewDimension:"2d"}},{binding:3,visibility:GPUShaderStage.FRAGMENT,texture:{sampleType:"float",viewDimension:"2d"}},{binding:4,visibility:GPUShaderStage.FRAGMENT,texture:{sampleType:"float",viewDimension:"2d"}},{binding:5,visibility:GPUShaderStage.FRAGMENT,texture:{sampleType:"float",viewDimension:"cube"}}]});const t=document.querySelector("#rocket_shader");if(!t)throw new Error("rocket_shader script element not found");const e=this.device.createShaderModule({label:"RocketShader",code:t.textContent||""});this.rocketPipeline=this.device.createRenderPipeline({label:"RocketPipeline",layout:this.device.createPipelineLayout({bindGroupLayouts:[this.rocketBindGroupLayout]}),vertex:{module:e,entryPoint:"vert_main",buffers:[{arrayStride:52,attributes:[{shaderLocation:0,offset:0,format:"float32x3"},{shaderLocation:1,offset:12,format:"float32x3"},{shaderLocation:2,offset:24,format:"float32x4"},{shaderLocation:3,offset:40,format:"float32x2"},{shaderLocation:4,offset:48,format:"float32"}]}]},fragment:{module:e,entryPoint:"frag_main",targets:[{format:"rgba16float"}]},primitive:{topology:"triangle-list",frontFace:"ccw",cullMode:"back"},depthStencil:{format:"depth24plus",depthWriteEnabled:!0,depthCompare:"less"}})}createExhaustPipeline(){this.exhaustBindGroupLayout=this.device.createBindGroupLayout({label:"ExhaustBindGroupLayout",entries:[{binding:0,visibility:GPUShaderStage.VERTEX|GPUShaderStage.FRAGMENT,buffer:{type:"uniform"}}]});const t=document.querySelector("#exhaust_shader");if(!t)throw new Error("exhaust_shader script element not found");const e=this.device.createShaderModule({label:"ExhaustShader",code:t.textContent||""}),r=this.device.createPipelineLayout({bindGroupLayouts:[this.exhaustBindGroupLayout]}),n=a=>({label:`ExhaustPipeline_${a}`,layout:r,vertex:{module:e,entryPoint:"vert_main",buffers:[{arrayStride:12,attributes:[{shaderLocation:0,offset:0,format:"float32x3"}]}]},fragment:{module:e,entryPoint:"frag_main",targets:[{format:"rgba16float",blend:{color:{srcFactor:"one",dstFactor:"one",operation:"add"},alpha:{srcFactor:"one",dstFactor:"one",operation:"add"}}}]},primitive:{topology:"triangle-list",frontFace:"ccw",cullMode:a},depthStencil:{format:"depth24plus",depthWriteEnabled:!1,depthCompare:"less"}});this.exhaustPipelineFront=this.device.createRenderPipeline(n("front")),this.exhaustPipelineBack=this.device.createRenderPipeline(n("back")),this.exhaustBindGroup=this.device.createBindGroup({label:"ExhaustBindGroup",layout:this.exhaustBindGroupLayout,entries:[{binding:0,resource:{buffer:this.exhaustUniformBuffer}}]})}createMipmapPipeline(){this.mipmapBindGroupLayout=this.device.createBindGroupLayout({label:"MipmapDownsampleBindGroupLayout",entries:[{binding:0,visibility:GPUShaderStage.FRAGMENT,texture:{sampleType:"float",viewDimension:"2d"}},{binding:1,visibility:GPUShaderStage.FRAGMENT,sampler:{type:"filtering"}}]});const t=this.device.createShaderModule({label:"MipmapShader",code:ee});this.mipmapPipeline=this.device.createRenderPipeline({label:"MipmapPipeline",layout:this.device.createPipelineLayout({bindGroupLayouts:[this.mipmapBindGroupLayout]}),vertex:{module:t,entryPoint:"vert_main"},fragment:{module:t,entryPoint:"frag_main",targets:[{format:"rgba16float"}]},primitive:{topology:"triangle-strip"}})}createRocketBuffers(t,e){this.rocketVertexBuffer=this.device.createBuffer({label:"RocketVertexBuffer",size:t.byteLength,usage:GPUBufferUsage.VERTEX|GPUBufferUsage.COPY_DST}),this.device.queue.writeBuffer(this.rocketVertexBuffer,0,t),this.rocketIndexBuffer=this.device.createBuffer({label:"RocketIndexBuffer",size:e.byteLength,usage:GPUBufferUsage.INDEX|GPUBufferUsage.COPY_DST}),this.device.queue.writeBuffer(this.rocketIndexBuffer,0,e),this.rocketIndexCount=e.length}createExhaustBuffers(){const e=new Float32Array(198);for(let n=0;n<=32;++n){const a=n==0?0:B,i=2*Math.PI*n/32;e[6*n]=a*Math.cos(i),e[6*n+1]=a*Math.sin(i),e[6*n+2]=A,e[6*n+3]=a*Math.cos(i),e[6*n+4]=a*Math.sin(i),e[6*n+5]=N}this.exhaustVertexBuffer=this.device.createBuffer({label:"ExhaustVertexBuffer",size:e.byteLength,usage:GPUBufferUsage.VERTEX|GPUBufferUsage.COPY_DST}),this.device.queue.writeBuffer(this.exhaustVertexBuffer,0,e);const r=new Uint32Array(384);for(let n=1;n<=32;++n){const a=n%32+1;r[12*n-12]=0,r[12*n-11]=2*a,r[12*n-10]=2*n,r[12*n-9]=2*n,r[12*n-8]=2*a,r[12*n-7]=2*a+1,r[12*n-6]=2*a+1,r[12*n-5]=2*n+1,r[12*n-4]=2*n,r[12*n-3]=1,r[12*n-2]=2*n+1,r[12*n-1]=2*a+1}this.exhaustIndexBuffer=this.device.createBuffer({label:"ExhaustIndexBuffer",size:r.byteLength,usage:GPUBufferUsage.INDEX|GPUBufferUsage.COPY_DST}),this.device.queue.writeBuffer(this.exhaustIndexBuffer,0,r),this.exhaustIndexCount=r.length}getRocketBindGroup(){const t=this.cameraView.textureManager,e=this.baseColorTextureWrapper.texture,r=this.occlusionRoughnessMetallicTextureWrapper.texture,n=this.normalMapTextureWrapper.texture;return this.rocketBindGroup&&this.cachedBaseColor===e&&this.cachedMetallic===r&&this.cachedNormal===n?this.rocketBindGroup:(this.cachedBaseColor=e,this.cachedMetallic=r,this.cachedNormal=n,this.rocketBindGroup=this.device.createBindGroup({label:"RocketBindGroup",layout:this.rocketBindGroupLayout,entries:[{binding:0,resource:{buffer:this.rocketUniformBuffer}},{binding:1,resource:t.linearSampler},{binding:2,resource:e.createView()},{binding:3,resource:r.createView()},{binding:4,resource:n.createView()},{binding:5,resource:this.envMapTexture.createView({dimension:"cube"})}]}),this.rocketBindGroup)}getEnvMapBindGroups(){const t=this.cameraView.textureManager,e=this.model.grid.getValue()?t.gridTexture:t.galaxyTexture;if(!e||!t.starTexture||!t.starTexture2||!t.blackbodyTexture||!t.dopplerTexture||!t.noiseTexture)return null;if(this.envMapBindGroups&&this.cachedEnvMapSkyTexture===e)return this.envMapBindGroups;this.cachedEnvMapSkyTexture=e,this.envMapBindGroups=[];for(let r=0;r<6;++r){const n=this.device.createBindGroup({label:`RocketEnvMapBindGroup_Face${r}`,layout:this.cameraView.bindGroupLayout,entries:[{binding:0,resource:{buffer:this.envMapUniformBuffers[r]}},{binding:1,resource:t.linearSampler},{binding:2,resource:t.rayDeflectionTexture.createView()},{binding:3,resource:t.rayInverseRadiusTexture.createView()},{binding:4,resource:e.createView({dimension:"cube"})},{binding:5,resource:t.starTexture.createView({dimension:"cube"})},{binding:6,resource:t.starTexture2.createView({dimension:"cube"})},{binding:7,resource:t.blackbodyTexture.createView()},{binding:8,resource:t.dopplerTexture.createView()},{binding:9,resource:t.noiseTexture.createView()},{binding:10,resource:t.nearestSampler}]});this.envMapBindGroups.push(n)}return this.envMapBindGroups}updateEnvMapUniforms(t,e,r,n){const a=this.model,i=new Float32Array(108);i[0]=a.t,i[1]=a.r,i[2]=a.worldTheta,i[3]=a.worldPhi,i[4]=a.p[0],i[5]=a.p[1],i[6]=a.p[2],i[7]=0,i[8]=a.kS[0],i[9]=a.kS[1],i[10]=a.kS[2],i[11]=a.kS[3],i[12]=a.rocketTau[1],i[13]=a.rocketTau[2],i[14]=a.rocketTau[3],i[15]=0,i[16]=e[0],i[17]=e[1],i[18]=e[2],i[19]=0,i[20]=r[0],i[21]=r[1],i[22]=r[2],i[23]=0,i[24]=n[0],i[25]=n[1],i[26]=n[2],i[27]=0,i[28]=a.starsMatrix[0],i[29]=a.starsMatrix[3],i[30]=a.starsMatrix[6],i[31]=0,i[32]=a.starsMatrix[1],i[33]=a.starsMatrix[4],i[34]=a.starsMatrix[7],i[35]=0,i[36]=a.starsMatrix[2],i[37]=a.starsMatrix[5],i[38]=a.starsMatrix[8],i[39]=0,i[40]=32,i[41]=32,i[42]=32,i[43]=0,i[44]=a.discDensity.getValue(),i[45]=a.discOpacity.getValue(),i[46]=a.discTemperature.getValue(),i[47]=0,i[48]=a.exposure.getValue(),i[49]=a.bloom.getValue();const p=a.grid.getValue()?0:this.cameraView.textureManager.getMinLoadedStarTextureLod();i[50]=p;const c=new Uint32Array(i.buffer);c[51]=a.lensing.getValue()?1:0,c[52]=a.doppler.getValue()?1:0,c[53]=a.grid.getValue()?1:0,c[54]=a.stars.getValue()?1:0,c[55]=a.highContrast.getValue()?1:0,i[56]=a.fovY,i[57]=0,i[58]=0,i[59]=0,i.set(this.cameraView.discParticles,60),this.device.queue.writeBuffer(this.envMapUniformBuffers[t],0,i)}setCameraUniforms(t,e){const r=this.model.cameraYaw.getValue()+this.model.cameraYawOffset-this.model.rocketYaw,n=this.model.rocketDistance.getValue()/2,a=.4*n,i=-a*Math.sin(this.model.rocketYaw),p=a*Math.cos(this.model.rocketYaw),c=Math.cos(r),s=Math.sin(r),d=Math.cos(this.model.cameraPitch.getValue()),l=Math.sin(this.model.cameraPitch.getValue()),v=[[c,0,-s,c*i-s*p],[-s*l,d,-c*l,-s*l*i-c*l*p],[s*d,l,c*d,-n+s*d*i+c*d*p],[0,0,0,1]],_=1/Math.tan(this.model.fovY/2),x=this.cameraView.canvas.width/this.cameraView.canvas.height,u=-100.1/(U-G),h=-2*U*G/(U-G),m=[[_/x,0,0,0],[0,_,0,0],[0,0,u,h],[0,0,-1,0]],g=new Float32Array(16);for(let T=0;T<4;++T)for(let y=0;y<4;++y){let C=0;for(let M=0;M<4;++M)C+=m[T][M]*v[M][y];g[y*4+T]=C}t.set(g);const b=[0,0,0];for(let T=0;T<3;++T)for(let y=0;y<3;++y)b[T]-=v[y][T]*v[y][3];e[0]=b[0],e[1]=b[1],e[2]=b[2]}renderEnvMap(t){const e=this.model,r=this.getEnvMapBindGroups();if(!r)return;const n=[{eW:[-e.rocketD[1],-e.rocketD[2],-e.rocketD[3]],eH:[-e.rocketH[1],-e.rocketH[2],-e.rocketH[3]],eD:[-e.rocketW[1],-e.rocketW[2],-e.rocketW[3]]},{eW:[e.rocketD[1],e.rocketD[2],e.rocketD[3]],eH:[-e.rocketH[1],-e.rocketH[2],-e.rocketH[3]],eD:[e.rocketW[1],e.rocketW[2],e.rocketW[3]]},{eW:[e.rocketW[1],e.rocketW[2],e.rocketW[3]],eH:[e.rocketD[1],e.rocketD[2],e.rocketD[3]],eD:[-e.rocketH[1],-e.rocketH[2],-e.rocketH[3]]},{eW:[e.rocketW[1],e.rocketW[2],e.rocketW[3]],eH:[-e.rocketD[1],-e.rocketD[2],-e.rocketD[3]],eD:[e.rocketH[1],e.rocketH[2],e.rocketH[3]]},{eW:[e.rocketW[1],e.rocketW[2],e.rocketW[3]],eH:[-e.rocketH[1],-e.rocketH[2],-e.rocketH[3]],eD:[-e.rocketD[1],-e.rocketD[2],-e.rocketD[3]]},{eW:[-e.rocketW[1],-e.rocketW[2],-e.rocketW[3]],eH:[-e.rocketH[1],-e.rocketH[2],-e.rocketH[3]],eD:[e.rocketD[1],e.rocketD[2],e.rocketD[3]]}];for(let a=0;a<6;++a){this.updateEnvMapUniforms(a,n[a].eW,n[a].eH,n[a].eD);const i=t.beginRenderPass({label:`RocketEnvMapPass_Face${a}`,colorAttachments:[{view:this.envMapTexture.createView({dimension:"2d",baseMipLevel:0,mipLevelCount:1,baseArrayLayer:a,arrayLayerCount:1}),loadOp:"clear",clearValue:{r:0,g:0,b:0,a:1},storeOp:"store"}]});i.setPipeline(this.cameraView.pipeline),i.setBindGroup(0,r[a]),i.setViewport(0,0,E,E,0,1),i.draw(4,1,0,0),i.end()}for(let a=1;a<L;++a){const i=E>>a;for(let p=0;p<6;++p){const c=this.device.createBindGroup({layout:this.mipmapBindGroupLayout,entries:[{binding:0,resource:this.envMapTexture.createView({dimension:"2d",baseMipLevel:a-1,mipLevelCount:1,baseArrayLayer:p,arrayLayerCount:1})},{binding:1,resource:this.cameraView.textureManager.linearSampler}]}),s=t.beginRenderPass({label:`RocketEnvMapMipmapPass_Level${a}_Face${p}`,colorAttachments:[{view:this.envMapTexture.createView({dimension:"2d",baseMipLevel:a,mipLevelCount:1,baseArrayLayer:p,arrayLayerCount:1}),loadOp:"clear",clearValue:{r:0,g:0,b:0,a:1},storeOp:"store"}]});s.setPipeline(this.mipmapPipeline),s.setBindGroup(0,c),s.setViewport(0,0,i,i,0,1),s.draw(4,1,0,0),s.end()}}}drawRocket(t){if(!this.rocketVertexBuffer||this.rocketIndexCount===0)return;const e=this.getRocketBindGroup();if(!e)return;const r=new Float32Array(20);this.setCameraUniforms(r.subarray(0,16),r.subarray(16,19)),this.device.queue.writeBuffer(this.rocketUniformBuffer,0,r),t.setPipeline(this.rocketPipeline),t.setBindGroup(0,e),t.setVertexBuffer(0,this.rocketVertexBuffer),t.setIndexBuffer(this.rocketIndexBuffer,"uint32"),t.drawIndexed(this.rocketIndexCount,1,0,0,0)}drawExhaust(t,e,r){if(!this.exhaustVertexBuffer||this.exhaustIndexCount===0)return;const n=.1*Math.pow(r,.75),a=[46/255*n,176/255*n,n],i=e*100,p=6.75+.5*Math.cos(i),c=5.75+.5*Math.cos((i+1)/Math.sqrt(2)),s=4.75+.5*Math.cos((i+2)/Math.sqrt(3)),d=B*B,l=[p/d,c/d,s/d],v=27+2*Math.cos((i+1)/Math.sqrt(2)),_=23+2*Math.cos((i+2)/Math.sqrt(3)),x=19+2*Math.cos(i),u=N-A,h=[v/u,_/u,x/u],m=new Float32Array(32);this.setCameraUniforms(m.subarray(0,16),m.subarray(16,19)),m[20]=a[0],m[21]=a[1],m[22]=a[2],m[24]=l[0],m[25]=l[1],m[26]=l[2],m[28]=h[0],m[29]=h[1],m[30]=h[2],this.device.queue.writeBuffer(this.exhaustUniformBuffer,0,m),t.setVertexBuffer(0,this.exhaustVertexBuffer),t.setIndexBuffer(this.exhaustIndexBuffer,"uint32"),t.setBindGroup(0,this.exhaustBindGroup),t.setPipeline(this.exhaustPipelineBack),t.drawIndexed(this.exhaustIndexCount,1,0,0,0),t.setPipeline(this.exhaustPipelineFront),t.drawIndexed(this.exhaustIndexCount,1,0,0,0)}}class ne{constructor(t,e,r){o(this,"model");o(this,"rootElement");o(this,"device");o(this,"devicePixelRatio");o(this,"canvas");o(this,"errorPanel");o(this,"errorPanelShown");o(this,"context");o(this,"canvasFormat");o(this,"uniformBuffer");o(this,"bindGroupLayout");o(this,"pipelineLayout");o(this,"pipeline",null);o(this,"bindGroup",null);o(this,"cachedSkyTexture",null);o(this,"textureManager");o(this,"shaderManager");o(this,"rocketManager");o(this,"bloom");o(this,"lastTauSeconds");o(this,"lastFrameTime");o(this,"numFrames",0);o(this,"drag",!1);o(this,"previousMouseX");o(this,"previousMouseY");o(this,"hidden",!1);o(this,"discParticles");this.model=t,this.rootElement=e,this.device=r,this.devicePixelRatio=this.getDevicePixelRatio();const n=e.querySelector("#camera_view"),a=e.querySelector("#cv_error_panel");if(!n)throw new Error("camera_view canvas not found");if(!a)throw new Error("cv_error_panel not found");this.canvas=n,this.canvas.style.width=`${e.clientWidth}px`,this.canvas.style.height=`${e.clientHeight}px`,this.canvas.width=e.clientWidth*this.devicePixelRatio,this.canvas.height=e.clientHeight*this.devicePixelRatio,this.errorPanel=a,this.errorPanelShown=!1;const i=this.canvas.getContext("webgpu");if(!i)throw new Error("Could not get WebGPU context");this.context=i,this.canvasFormat=navigator.gpu.getPreferredCanvasFormat(),this.context.configure({device:this.device,format:this.canvasFormat,alphaMode:"opaque"}),this.uniformBuffer=this.device.createBuffer({size:432,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST}),this.bindGroupLayout=this.device.createBindGroupLayout({entries:[{binding:0,visibility:GPUShaderStage.VERTEX|GPUShaderStage.FRAGMENT,buffer:{type:"uniform"}},{binding:1,visibility:GPUShaderStage.FRAGMENT,sampler:{type:"filtering"}},{binding:2,visibility:GPUShaderStage.FRAGMENT,texture:{sampleType:"float",viewDimension:"2d"}},{binding:3,visibility:GPUShaderStage.FRAGMENT,texture:{sampleType:"float",viewDimension:"2d"}},{binding:4,visibility:GPUShaderStage.FRAGMENT,texture:{sampleType:"float",viewDimension:"cube"}},{binding:5,visibility:GPUShaderStage.FRAGMENT,texture:{sampleType:"float",viewDimension:"cube"}},{binding:6,visibility:GPUShaderStage.FRAGMENT,texture:{sampleType:"float",viewDimension:"cube"}},{binding:7,visibility:GPUShaderStage.FRAGMENT,texture:{sampleType:"float",viewDimension:"2d"}},{binding:8,visibility:GPUShaderStage.FRAGMENT,texture:{sampleType:"float",viewDimension:"3d"}},{binding:9,visibility:GPUShaderStage.FRAGMENT,texture:{sampleType:"float",viewDimension:"2d"}},{binding:10,visibility:GPUShaderStage.FRAGMENT,sampler:{type:"non-filtering"}}]}),this.pipelineLayout=this.device.createPipelineLayout({bindGroupLayouts:[this.bindGroupLayout]}),this.textureManager=new Q(e,this.device),this.shaderManager=new J(t,this.textureManager,this.device),this.rocketManager=new re(t,this),this.bloom=new $(this.device,this.canvasFormat,this.canvas.width,this.canvas.height),this.lastTauSeconds=Date.now()/1e3,this.lastFrameTime=void 0,this.numFrames=0,this.drag=!1,this.previousMouseX=void 0,this.previousMouseY=void 0,this.hidden=!1;const p=3,c=12,s=function(_,x,u){const h=(x-_)/(u-_),m=1e4;let g=0;for(let b=0;b<m;++b){const T=1/m,y=(b+.5)/m;g+=T/Math.sqrt((1-y*y)*(1-h*y*y))}return Math.PI*Math.sqrt(u-_)/(4*g)};this.discParticles=new Float32Array(48);let d=0,l=42;const v=()=>{let _=Math.sin(l++)*1e4;return _-Math.floor(_)};for(let _=p;_<c;_+=.75){const x=.1*v(),h=1/(_*(1+x)/(1-x)),m=1/_,g=1-h-m,b=2*Math.PI*v(),T=s(h,m,g);this.discParticles[d++]=h,this.discParticles[d++]=m,this.discParticles[d++]=b,this.discParticles[d++]=T}window.addEventListener("mousedown",_=>this.onMouseDown(_)),window.addEventListener("mousemove",_=>this.onMouseMove(_)),window.addEventListener("mouseup",()=>this.onMouseUp()),window.addEventListener("resize",()=>this.onResize()),document.addEventListener("visibilitychange",()=>{this.hidden=document.hidden,this.hidden||(this.lastFrameTime=void 0)}),requestAnimationFrame(()=>this.onRender())}getBindGroup(){const t=this.textureManager,e=t.rayDeflectionTexture,r=t.rayInverseRadiusTexture,n=t.noiseTexture,a=t.starTexture,i=t.starTexture2,p=t.blackbodyTexture,c=t.dopplerTexture;if(!e||!r||!n||!a||!i||!p||!c)return null;const s=this.model.grid.getValue()?this.textureManager.gridTexture:this.textureManager.galaxyTexture;return s?this.bindGroup&&this.cachedSkyTexture===s?this.bindGroup:(this.cachedSkyTexture=s,this.bindGroup=this.device.createBindGroup({layout:this.bindGroupLayout,entries:[{binding:0,resource:{buffer:this.uniformBuffer}},{binding:1,resource:this.textureManager.linearSampler},{binding:2,resource:e.createView()},{binding:3,resource:r.createView()},{binding:4,resource:s.createView({dimension:"cube"})},{binding:5,resource:a.createView({dimension:"cube"})},{binding:6,resource:i.createView({dimension:"cube"})},{binding:7,resource:p.createView()},{binding:8,resource:c.createView()},{binding:9,resource:n.createView()},{binding:10,resource:this.textureManager.nearestSampler}]}),this.bindGroup):null}updateUniforms(){const t=this.model,e=new Float32Array(108);e[0]=t.t,e[1]=t.r,e[2]=t.worldTheta,e[3]=t.worldPhi,e[4]=t.p[0],e[5]=t.p[1],e[6]=t.p[2],e[7]=0,e[8]=t.kS[0],e[9]=t.kS[1],e[10]=t.kS[2],e[11]=t.kS[3],e[12]=t.eTau[1],e[13]=t.eTau[2],e[14]=t.eTau[3],e[15]=0,e[16]=t.eW[1],e[17]=t.eW[2],e[18]=t.eW[3],e[19]=0,e[20]=t.eH[1],e[21]=t.eH[2],e[22]=t.eH[3],e[23]=0,e[24]=t.eD[1],e[25]=t.eD[2],e[26]=t.eD[3],e[27]=0,e[28]=t.starsMatrix[0],e[29]=t.starsMatrix[3],e[30]=t.starsMatrix[6],e[31]=0,e[32]=t.starsMatrix[1],e[33]=t.starsMatrix[4],e[34]=t.starsMatrix[7],e[35]=0,e[36]=t.starsMatrix[2],e[37]=t.starsMatrix[5],e[38]=t.starsMatrix[8],e[39]=0;const r=Math.tan(t.fovY/2),n=this.canvas.height/(2*r);e[40]=this.canvas.width/2,e[41]=this.canvas.height/2,e[42]=n,e[43]=0,e[44]=t.discDensity.getValue(),e[45]=t.discOpacity.getValue(),e[46]=t.discTemperature.getValue(),e[47]=0,e[48]=t.exposure.getValue(),e[49]=t.bloom.getValue();const a=t.grid.getValue()?0:this.textureManager.getMinLoadedStarTextureLod();e[50]=a;const i=new Uint32Array(e.buffer);i[51]=t.lensing.getValue()?1:0,i[52]=t.doppler.getValue()?1:0,i[53]=t.grid.getValue()?1:0,i[54]=t.stars.getValue()?1:0,i[55]=t.highContrast.getValue()?1:0,e[56]=t.fovY,e[57]=0,e[58]=0,e[59]=0,e.set(this.discParticles,60),this.device.queue.writeBuffer(this.uniformBuffer,0,e)}onRender(){if(this.hidden)return;const t=this.shaderManager.getProgram();if(!t){requestAnimationFrame(()=>this.onRender());return}this.pipeline||(this.pipeline=this.device.createRenderPipeline({label:"BlackHoleRenderPipeline",layout:this.pipelineLayout,vertex:{module:t,entryPoint:"vert_main"},fragment:{module:t,entryPoint:"frag_main",targets:[{format:"rgba16float"}]},primitive:{topology:"triangle-strip"}}));const e=this.getBindGroup();if(!e){requestAnimationFrame(()=>this.onRender());return}const r=Date.now()/1e3,n=r-this.lastTauSeconds;this.lastTauSeconds=r,this.updateUniforms();const a=this.device.createCommandEncoder(),i=this.context.getCurrentTexture().createView();this.model.rocket.getValue()&&this.rocketManager.renderEnvMap(a);const p=this.bloom.begin(),c={colorAttachments:[{view:p,clearValue:{r:0,g:0,b:0,a:1},loadOp:"clear",storeOp:"store"}]},s=a.beginRenderPass(c);if(s.setPipeline(this.pipeline),s.setBindGroup(0,e),s.setViewport(1,1,this.canvas.width,this.canvas.height,0,1),s.draw(4,1,0,0),s.end(),this.model.rocket.getValue()){const d={colorAttachments:[{view:p,loadOp:"load",storeOp:"store"}],depthStencilAttachment:{view:this.bloom.depthTexture.createView(),depthClearValue:1,depthLoadOp:"clear",depthStoreOp:"discard"}},l=a.beginRenderPass(d);l.setViewport(1,1,this.canvas.width,this.canvas.height,0,1),this.rocketManager.drawRocket(l),this.model.gForce>0&&this.rocketManager.drawExhaust(l,r,this.model.gForce),l.end()}this.bloom.end(a,i,this.model.bloom.getValue(),this.model.exposure.getValue(),this.model.highContrast.getValue()),this.device.queue.submit([a.finish()]),this.model.updateOrbit(n),requestAnimationFrame(()=>this.onRender()),this.checkFrameRate()}checkFrameRate(){const t=Date.now();this.lastFrameTime||(this.lastFrameTime=t,this.numFrames=0),this.numFrames+=1,t>this.lastFrameTime+1e3&&(this.numFrames<=10&&this.model.stars.getValue()&&!this.errorPanelShown&&(this.model.stars.setValue(!1),this.errorPanel.innerHTML="Stars have been automatically disabled to improve performance. You can re-enable them from the left hand side panel.",this.errorPanel.classList.toggle("cv-hidden",!1),this.errorPanel.classList.toggle("cv-warning",!0),this.errorPanelShown=!0),this.lastFrameTime=t,this.numFrames=0)}onMouseDown(t){this.previousMouseX=t.screenX,this.previousMouseY=t.screenY;const e=t.target;this.drag=e.tagName!="INPUT"&&!t.ctrlKey}onMouseMove(t){const e=t.screenX,r=t.screenY;if(this.drag){let a=this.model.cameraYaw.getValue(),i=this.model.cameraPitch.getValue();const p=this.previousMouseX??e,c=this.previousMouseY??r;a+=(p-e)/500,i-=(c-r)/500,this.model.cameraYaw.setValue(a-2*Math.PI*Math.floor(a/(2*Math.PI))),this.model.cameraPitch.setValue(i)}this.previousMouseX=e,this.previousMouseY=r}onMouseUp(){this.drag=!1}onResize(){const t=this.rootElement;this.devicePixelRatio=this.getDevicePixelRatio(),this.canvas.style.width=`${t.clientWidth}px`,this.canvas.style.height=`${t.clientHeight}px`,this.canvas.width=t.clientWidth*this.devicePixelRatio,this.canvas.height=t.clientHeight*this.devicePixelRatio,this.bloom.resize(this.canvas.width,this.canvas.height)}getDevicePixelRatio(){return this.model.highDefinition.getValue()?window.devicePixelRatio:1}}function I(f,t,e){let r=document.getElementById(f);r||(r=document.createElement("script"),r.id=f,r.setAttribute("type",t),document.body.appendChild(r)),r.textContent=e}I("black_hole_shader","text/wgsl",Y);I("rocket_shader","text/wgsl",X);I("exhaust_shader","text/wgsl",j);window.addEventListener("DOMContentLoaded",async()=>{if(!navigator.gpu){const s=document.querySelector("#cv_error_panel");s&&(s.innerHTML="WebGPU is not supported in this browser. Please use a WebGPU-enabled browser (like Chrome or Edge).",s.classList.toggle("cv-hidden",!1));return}const f=await navigator.gpu.requestAdapter();if(!f){const s=document.querySelector("#cv_error_panel");s&&(s.innerHTML="Failed to request WebGPU adapter.",s.classList.toggle("cv-hidden",!1));return}const t=[];f.features.has("float32-filterable")&&t.push("float32-filterable");const e=await f.requestDevice({requiredFeatures:t}),r=new q;new z(r);const n=new URLSearchParams(window.location.search),a=n.has("hide_menu")&&n.get("hide_menu")!=="false"&&n.get("hide_menu")!=="0",i=n.has("hide_orbit")&&n.get("hide_orbit")!=="false"&&n.get("hide_orbit")!=="0",p=document.body.querySelector("#settings_panel");p&&new H(p,r,a);const c=document.body.querySelector("#orbit_panel");c&&new W(c,r,i),window.addEventListener("error",s=>{const d=document.querySelector("#cv_error_panel");d&&(d.innerHTML="JS Error: "+s.message,d.classList.toggle("cv-hidden",!1))}),window.addEventListener("unhandledrejection",s=>{const d=document.querySelector("#cv_error_panel");d&&(d.innerHTML="Promise Error: "+s.reason,d.classList.toggle("cv-hidden",!1))}),new ne(r,document.body,e)});
